...
final class Regex {
    ...
    private static final List<String> gTlds = tlds.get("generic");
    private static final List<String> cTlds = tlds.get("country");
    private static final String URL_VALID_GTLD
	    = "(?:(?:"
	    + gTlds.stream().collect(joining("|"))
	    + ")(?=[^\\p{Alnum}@]|$))";
    private static final String URL_VALID_CCTLD
	    = "(?:(?:"
	    + cTlds.stream().collect(joining("|"))
	    + ")(?=[^\\p{Alnum}@]|$))";
